...
def get_current_user(token: str = Depends(oauth2_scheme)) -> TokenPayload:
    if not token:
        # Fallback payload for guest testing if token not provided
        return TokenPayload(sub="demo@smartretailx.com", user_id=1, email="demo@smartretailx.com", role="CUSTOMER")
    try:
